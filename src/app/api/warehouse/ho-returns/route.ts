...
interface StockRow { item_no: string; in_stock: number; description: string | null }
interface DocLine { item_no: string; description: string | null; qty: number; current: number }
interface Doc { doc: string; store: string; date: string | null; applied: boolean; lines: DocLine[]; totalQty: number }
...
export async function GET() {
  try {
    const [tRes, stockRows, refs] = await Promise.all([
      ...
      query<StockRow>("SELECT item_no, in_stock, description FROM warehouse_stock"),
      query<{ source_ref: string }>("SELECT DISTINCT source_ref FROM wh_stock_ledger WHERE type='return_in'"),
    ]);

    const stock = new Map(stockRows.map(r => [String(r.item_no).trim(), { in_stock: Number(r.in_stock) || 0, desc: r.description }]));
    ...
    for (const r of tRes.value) {
      ...
      if (!d) { d = { doc: r.doc, store: r.frm, date: r.sd ? String(r.sd).slice(0, 10) : null, applied: applied.has(r.doc), lines: [], totalQty: 0 }; byDoc.set(r.doc, d); }
      const s = stock.get(item);
      const ex = d.lines.find(x => x.item_no === item);
      if (ex) ex.qty += qty;
      else d.lines.push({ item_no: item, description: s?.desc ?? null, qty, current: s?.in_stock ?? 0 });
      d.totalQty += qty;
    }
