...
interface LineRow { doc: string; cust: string; pd: string; item: string; descr: string; qty: number }
interface StockRow { item_no: string; in_stock: number }
interface RefRow { source_ref: string }

interface DocLine { item_no: string; description: string; qty: number; current: number }
interface Doc { doc: string; cust: string; date: string; applied: boolean; overridden: boolean; lines: DocLine[]; totalQty: number; anyNegative: boolean }
...
const LINES = (tbl: string) => `
  SELECT [Document No_] AS doc, [Sell-to Customer No_] AS cust,
         ...
         [No_] AS item, [Description] AS descr, [Quantity] AS qty
    FROM ${tbl}
   WHERE [Type] = 2 AND [Quantity] <> 0
     AND CAST([Posting Date] AS DATE) BETWEEN @from AND @to
   ORDER BY [Posting Date] DESC, [Document No_]`;

function group(rows: LineRow[], stock: Map<string, number>, applied: Set<string>, overridden: Set<string>, sign: number): Doc[] {
  ...
  for (const r of rows) {
    ...
    if (!d) {
      d = { doc: r.doc, cust: r.cust, date: String(r.pd).slice(0, 10), applied: applied.has(r.doc), overridden: overridden.has(r.doc), lines: [], totalQty: 0, anyNegative: false };
      byDoc.set(r.doc, d);
    }
    const ex = d.lines.find(x => x.item_no === item);
    if (ex) ex.qty += qty;
    else d.lines.push({ item_no: item, description: (r.descr || "").trim(), qty, current: stock.get(item) ?? 0 });
    d.totalQty += qty;
  }
...
export async function GET(req: NextRequest) {
  ...
  try {
    const [invRes, cmRes, stockRows, invRefs, cmRefs, ovRows] = await Promise.all([
    ...
    const invoices = group(invRes.value, stock, invApplied, overridden, -1);
    const creditMemos = group(cmRes.value, stock, cmApplied, overridden, +1);
